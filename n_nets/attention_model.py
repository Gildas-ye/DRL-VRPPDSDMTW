...
from my_utils.tensor_functions import compute_in_batches

from n_nets.graph_encoder import GraphAttentionEncoder
from torch.nn import DataParallel
from my_utils.beam_search import CachedLookup
from my_utils.functions import sample_many
import copy
import random
...
def the_ids(cls: tuple):
    cls.keys = lambda self: self.state.ids.view(-1)
...
class AttentionModel(nn.Module):
    ...
    def _calc_log_likelihood(self, _log_p, _log_p_veh, a, mask, veh_list):  # a is pi
        ...
        # TODO gildas a touché là. j'ai enlevé .cuda
    ...
    def _init_embed(self, input):
        ...
        if self.is_hcvrp:
            ...
            demand = torch.tensor([(input['demand'] / input['capacity'][0:1, veh]).tolist() for veh in range(input['capacity'].size(-1))]).transpose(0, 1).transpose(1, 2) #.cuda()
            ...
            if from_depot:
                ...
                return torch.cat(  # [batch_size, num_veh, embed_dim+1]
                    (
                        ...
                        (torch.tensor(self.problem.VEHICLE_CAPACITY)[None, veh] - state.used_capacity[
                            torch.arange(batch_size), veh]).transpose(0, 1).unsqueeze(-1)
                    ),
                    -1
                )
            # TODO gildas a touché la. j'ai enlevé un .cuda
